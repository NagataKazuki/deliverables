...
static class LensLaws
{
    ...
    public static void NEG(Domain S, Domain V, VarPool vars, Cnf cnf)
    {
        cnf.Add(
            S.Elems
             .SelectMany(
                 s => V.Elems.Select(
                     v => vars.Get(new GKey(s, v))
                 )
             )
             .ToArray()
        );
    ...
    public static void NEP(Domain S, Domain V, VarPool vars, Cnf cnf)
    {
        cnf.Add(
            S.Elems
             .SelectMany(
                 s => V.Elems.SelectMany(
                     v => S.Elems.Select(
                         s2 => vars.Get(new PKey(s, v, s2))
                     )
                 )
             )
             .ToArray()
    ...
    public static void NEP2(Domain S, Domain V, VarPool vars, Cnf cnf)
    {
    ...
    public static void NEP3(Domain S, Domain V, VarPool vars, Cnf cnf)
    {
        foreach (var v in V.Elems)
        {
            cnf.Add(
